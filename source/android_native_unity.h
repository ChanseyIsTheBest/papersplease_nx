...
/* Re-assert the window geometry if something reset it behind our back.
 * Safe to call every frame; a no-op once the sizes agree. */
void android_native_reassert_geom(void);

/* The REAL window (always the landscape panel), as opposed to
 * android_native_width/height which are what the ENGINE is told. */
u32 android_native_window_width(void);
u32 android_native_window_height(void);

/* Reported display dpi, resolution-scaled so the derived physical size is the
 * same docked and handheld. See android_native_unity.c. */
