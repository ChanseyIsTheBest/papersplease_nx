...
#include <malloc.h>
#include "config.h"
#include "nx_gameroot.h"   /* runtime game folder (was hardcoded GAME_HOME) */
#include <wchar.h>
#include <wctype.h>
#include <time.h>
#include <sys/stat.h>
...
static void mkdir_p_dir(const char *dir) {
  ...
  size_t skip;
  const char *groot = game_root();          /* runtime, not baked in */
  const size_t glen = strlen(groot);
  if (strncmp(tmp, groot, glen) == 0 && (tmp[glen] == '/' || tmp[glen] == '\0')) {
...
int g_watch_fd = -1;   /* bundle/asset fd: trace its reads/seeks to debug header load */
...
static int synth_proc_open(const char *path) {
  ...
  safe[j] = '\0';
  char tf[256];
  snprintf(tf, sizeof tf, "%s/.synth%s", game_root(), safe);
...
static const char *casetest_redirect(const char *path) {
  ...
  if (strncasecmp(b, "CASESENSITIVETEST", 17) == 0) {
    /* Built once into static storage: the game root is only known at runtime
     * now, so this can no longer be a string literal. */
    static char casetest[288];
    if (!casetest[0]) snprintf(casetest, sizeof casetest, "%s/.casetest", game_root());
    return casetest;
  }
  return path;
}

/* Redirect game-root-relative absolute paths onto the real game folder.
 *
 * Papers, Please builds its localisation path as "/assets/loc/<code>.zip" --
 * absolute from the filesystem ROOT, because on Android the StreamingAssets base
 * it concatenates resolves to the APK's assets/ directory. There is no such root
 * here, so the open failed:
 *
 *     [io] open(/assets/loc/en.zip,0x0) -> -1
 *     Unity: [Res] Language [en] load failed: NullReferenceException
 *       at Res.loadLanguage (System.String languageCode, ...)
 *
 * and the game retried forever -- the boot-blocking exception loop. It was never
 * a JNI problem at all.
 *
 * The game's data really is at <gameroot>/assets/..., so map a leading
 * "/assets/" (and the engine's own "/bin/Data/") onto it. Only used when the
 * literal path does not exist, so a real absolute path is never shadowed.
 *
 * Returns buf on rewrite, or the original pointer when nothing applies. */
static const char *root_relative_redirect(const char *path, char *buf, size_t sz) {
  if (!path || path[0] != '/') return path;

  static const char *const kPrefixes[] = { "/assets/", "/bin/Data/", "/loc/" };
  for (unsigned i = 0; i < sizeof kPrefixes / sizeof kPrefixes[0]; i++) {
    const size_t n = strlen(kPrefixes[i]);
    if (strncmp(path, kPrefixes[i], n) != 0) continue;

    /* "/bin/Data/..." and "/loc/..." are below assets/; "/assets/..." is not. */
    if (kPrefixes[i][1] == 'a')
      snprintf(buf, sz, "%s%s", game_root(), path);
    else
      snprintf(buf, sz, "%s/assets%s", game_root(), path);

    struct stat st;
    if (stat(buf, &st) == 0) {
      static int logged = 0;
      if (logged < 24) { logged++; debugPrintf("[io] redirect %s -> %s\n", path, buf); }
      return buf;
    }
    return path;                      /* no such file either way; let it fail honestly */
  }
  return path;
}

/* External wrapper so other translation units (imports.c's access()) can use the
 * same redirect without duplicating the rules. */
const char *nx_root_relative(const char *path, char *buf, unsigned long sz) {
  return root_relative_redirect(path, buf, (size_t)sz);
}

int open_fake(const char *path, int flags, ...) {
  path = casetest_redirect(path);
  char rrbuf[512];
  path = root_relative_redirect(path, rrbuf, sizeof rrbuf);
  int mode = 0666;
  ...
  if (!writing) {
    ...
    if (!strcmp(path, "/dev/urandom") || !strcmp(path, "/dev/random")) {
      ...
      char tf[256];
      snprintf(tf, sizeof tf, "%s/.synth_dev_random", game_root());
      int wfd = open(tf, O_WRONLY | O_CREAT | O_TRUNC, 0644);
  ...
  if (fd >= 0) {
    ...
    /* The base watched data.unity3d, which this game does not ship. Watch the
     * files it DOES load its scene data from, so TRACE_BUNDLE_IO stays useful. */
    if (strstr(path, "data.unity3d") || strstr(path, "sharedassets0.assets") ||
        strstr(path, "globalgamemanagers.assets") ||
        /* localisation zips: Res.loadLanguage opens one and then fails, and we
         * currently cannot see whether it ever READS it. Watching these makes
         * the difference between "open succeeded but nothing was read" and
         * "read fine, the parse failed" visible in one line. */
        strstr(path, "/loc/")) {
      g_watch_fd = fd;
      debugPrintf("[io] >>> watching fd=%d (%s)\n", fd, path);
    }
  } else {
...
int stat_fake(const char *path, struct bionic_stat *st) {
  path = casetest_redirect(path);
  /* Same root-relative redirect as open_fake/fopen_fake. This one matters just
   * as much: the managed file layer stats a path BEFORE reading it (File.Exists,
   * FileStream length). Redirecting only open() meant the read could succeed
   * while the preceding stat still reported "missing", so the caller took its
   * not-found path and nulled out -- which is what kept Res.loadLanguage failing
   * even after the open started working. */
  char rrbuf[512];
  path = root_relative_redirect(path, rrbuf, sizeof rrbuf);
...
FILE *fopen_fake(const char *path, const char *mode) {
  /* Same root-relative redirect as open_fake: the game reaches its localisation
   * zips through stdio as well as raw open(). See root_relative_redirect(). */
  char rrbuf[512];
  path = root_relative_redirect(path, rrbuf, sizeof rrbuf);
  path = casetest_redirect(path);
...
void *getpwuid_fake(int uid) {
  ...
  /* pw_dir must be writable storage filled at runtime -- a static initialiser
   * cannot call game_root(). */
  static char dir[256];
  if (!dir[0]) snprintf(dir, sizeof dir, "%s", game_root_unix());
...
char *getenv_fake(const char *name) {
  if (name) {
    if (!strcmp(name, "HOME"))   return (char *)game_root_unix();
    if (!strcmp(name, "TMPDIR")) return (char *)game_root_unix();
  }
  return getenv(name);
}
...
void *dlsym_fake(void *handle, const char *symbol) {
  ...
  /* Native P/Invoke stub resolver (pp_social_stub.c) -- Papers, Please ships
   * Google Play Games, not Firebase; see that file. */
  extern void *pp_native_stub_lookup(const char *symbol);
  ...
  void *fb = pp_native_stub_lookup(symbol);
  if (fb) return fb;
...
/* Re-derived for PAPERS, PLEASE's libil2cpp.so.  These MUST match the game's
 * own binary -- they are the single most port-specific values in the tree, and
 * pointing them at another build's globals is the classic cause of a black
 * screen with the watchdog reporting "last frame=0".  Full derivation, with the
 * disassembly they came from, is in pp_il2cpp.h. */
#include "pp_il2cpp.h"
#define GC_SUSPEND_SIG_OFF PP_GC_SUSPEND_SIG_OFF   /* 0xf52a94 */
#define GC_RESTART_SIG_OFF PP_GC_RESTART_SIG_OFF   /* 0xf52a98 */
#define GC_START_ACK_OFF   PP_GC_START_ACK_OFF     /* 0xf52a90 */
#define GC_ACK_SEM_OFF     PP_GC_ACK_SEM_OFF       /* 0x116b9a0 */
