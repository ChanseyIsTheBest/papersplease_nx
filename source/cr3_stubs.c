 ...
 *
 * For PAPERS, PLEASE these handlers are DEAD CODE: jni_fake.c now delegates every
...
/* ---- data layer ---- */
/* The il2cpp runtime uses this as its persistentDataPath: it extracts and mmaps
 * unity.ver / global-metadata.dat / Resources/ underneath it, and Papers,
 * Please's HostUnity.UnityPlatformDisk puts its SAVE FILES there too. Leaving
 * the base's hardcoded path here is a real bug (the PvZ port hit it): the game
 * silently reads and writes under another game's folder. Use GAME_HOME. */
const char *data_dir(void) { return game_root(); }
